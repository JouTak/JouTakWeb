    ...
    def test_v2_documents_preserve_primary_legacy_user_actions(self):
        ...
        for endpoint, expected_values in expected.items():
            with self.subTest(endpoint=endpoint):
                ...
                destinations = set()
                for item in actions[0].items:
                    self.assertIn(item.action.kind, ("internal", "external"))
                    destinations.add(
                        item.action.path
                        if item.action.kind == "internal"
                        else str(item.action.href)
                    )
                destinations.update(fact.value for fact in actions[0].facts)
                self.assertEqual(destinations, expected_values)
