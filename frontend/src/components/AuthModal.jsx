...
export default function AuthModal({
  ...
  function validateLogin() {
    if (!loginEmail.trim()) return "Укажите email.";
    if (!emailOk(loginEmail.trim())) return "Неверный формат email.";
  ...
  function validateSignup() {
    ...
    if (!emailOk(suEmail.trim())) return "Неверный формат email.";
  ...
  async function onLoginSubmit(e) {
    ...
    setBusy(true);
    try {
      await doLogin({ email: loginEmail.trim(), password });
      const p = await me();
      toaster.add({
        title: "Готово!",
      ...
      if (needsPersonalization(p)) {
        toaster.add({
          ...
          content:
            "Чтобы открыть полный функционал, заполните обязательные поля профиля.",
          theme: "warning",
        });
          ...
          <form onSubmit={onLoginSubmit} style={{ display: "grid", gap: 12 }}>
            <div style={fieldBlockStyle}>
              ...
              <TextInput
                size="l"
                value={loginEmail}
                onUpdate={setLoginEmail}
                name="joutak__login_email"
                autoComplete="email"
                autoFocus
                disabled={busy}
                aria-label="Email"
          ...
          <form onSubmit={onSignupSubmit} style={{ display: "grid", gap: 12 }}>
            ...
            <div style={fieldBlockStyle}>
              ...
              <TextInput
                ...
                name="joutak__signup_password_repeat"
                autoComplete="new-password"
                disabled={busy}
