    ...
    async function onAddPasskey() {
        ...
        try {
            ...
            if (e?.name === 'NotAllowedError') {
                ...
                setMsg('Нужно повторно войти (reauth) или активировать бридж /api/bridge/headless/login.');
            } else if (e?.response?.status === 401) {
                setMsg('Нужно подтвердить действие (reauth). Откройте заново страницу безопасности.');
            } else {
                        ...
                        <div key={a.id} style={{
                            ...
                            <div style={{display: 'grid', gap: 4}}>
                                ...
                                <div style={{opacity: 0.7, fontSize: 12}}>
                                    Создан: {a.created_at ? new Date(a.created_at * 1000).toLocaleString() : '—'}
                                    {a.last_used_at ? ` · Последнее использование: ${new Date(a.last_used_at * 1000).toLocaleString()}` : ''}
                                </div>
                                <div style={{display: 'flex', gap: 8, marginTop: 6}}>
                                    <TextInput
                                        ...
                                        onUpdate={(v) => setNameDrafts((m) => ({...m, [a.id]: v}))}
                                    />
                                    <Button view="outlined" size="m" onClick={() => onRename(a.id)}
