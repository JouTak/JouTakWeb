...
const COMMON_PASSWORDS = new Set([
  ...
  '123123','000000','iloveyou','1234','1q2w3e4r','qwertyuiop','admin','monkey',
  'letmein','dragon','sunshine','princess'
]);
...
function computeStrength(pwd, {minLength, username}) {
  ...
  return {score, label, flags:{lenOK,hasLower,hasUpper,hasDigit,hasSpec,notOnlyDigits,notCommon,notSimilar}};
}
...
function StrengthBar({score, label}) {
  ...
  return (
    <div style={{display:'grid', gap:6}}>
      <div aria-hidden style={{height:8, background:'rgba(255,255,255,0.08)', borderRadius:6}}>
        <div style={{width:`${percent}%`,height:'100%',background:bg,borderRadius:6,transition:'width 200ms ease'}}/>
      </div>
...
export default function PasswordCard({
  ...
  const strength = useMemo(
    () => (editing ? computeStrength(new1, {minLength, username}) : {score:0,label:'Пусто'}),
  ...
  const {curErr, n1Err, n2Err, hasErrors} = useMemo(() => {
    ...
    if (touched.n1) {
      ...
      else if (new1.length < minLength) n1Err = `Минимум ${minLength} символов.`;
      else if (/^\d+$/.test(new1)) n1Err = 'Пароль не должен состоять только из цифр.';
      else if (username) {
        const uname = username.toLowerCase();
        const unameLocal = uname.includes('@') ? uname.split('@')[0] : uname;
        if (unameLocal && new1.toLowerCase().includes(unameLocal)) n1Err = 'Пароль слишком похож на логин/почту.';
      }
      if (!n1Err && COMMON_PASSWORDS.has(new1.toLowerCase())) n1Err = 'Слишком распространённый пароль.';
    }
    if (touched.n2) {
  ...
  const resetForm = useCallback(() => {
    setCurPwd(''); setNew1(''); setNew2('');
    setTouched({cur:false, n1:false, n2:false});
    setBackendErr({cur:'', n1:''});
    setCapsCur(false); setCapsNew(false);
  }, []);
  ...
  function mapBackendErrors(data) {
    let cur = ''; let n1 = '';
    const fieldMsgs = (k) =>
      Array.isArray(data?.[k]) ? data[k].join('\n') :
      (typeof data?.[k] === 'string' ? data[k] : '');
    n1 = fieldMsgs('new_password') || fieldMsgs('password') || '';
    cur = fieldMsgs('current_password') || '';
    if (/too short/i.test(n1)) n1 = `Слишком короткий пароль (минимум ${minLength} символов).`;
    if (/too common/i.test(n1)) n1 = 'Слишком распространённый пароль.';
    if (/entirely numeric/i.test(n1)) n1 = 'Пароль не должен состоять только из цифр.';
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      try {
        if (username && 'credentials' in navigator && 'PasswordCredential' in window) {
          const cred = new window.PasswordCredential({id: String(username), password: new1});
    ...
    <section style={cardStyle}>
      ...
      {!editing ? (
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap'}}>
          <div style={{opacity:0.8}}>Для безопасности поля скрыты. Нажмите, чтобы изменить пароль.</div>
        ...
        >
          ...
          <TextInput
            ...
            value={curPwd}
            onUpdate={(v) => { setCurPwd(v); if (!touched.cur) setTouched((t)=>({...t, cur:true})); }}
            onBlur={() => setTouched((t)=>({...t, cur:true}))}
            onKeyUp={(e) => setCapsCur(e.getModifierState && e.getModifierState('CapsLock'))}
            name="current-password"
            autoComplete="current-password"
            validationState={curErr ? 'invalid' : undefined}
            errorMessage={curErr || undefined}
          />
          {capsCur && <div style={{color:'#c47f1b', fontSize:12}}>Внимание: включён Caps Lock.</div>}
          ...
          <TextInput
            ...
            value={new1}
            onUpdate={(v) => { setNew1(v); if (!touched.n1) setTouched((t)=>({...t, n1:true})); }}
          ...
          />
          {capsNew && <div style={{color:'#c47f1b', fontSize:12}}>Внимание: включён Caps Lock.</div>}
          ...
          <TextInput
            ...
            value={new2}
            onUpdate={(v) => { setNew2(v); if (!touched.n2) setTouched((t)=>({...t, n2:true})); }}
          ...
          <div style={{opacity:0.8, fontSize:12, display:'grid', gap:4}}>
            <div>Требования: минимум {minLength} символов, пароль не должен быть полностью из цифр и не должен совпадать с логином/почтой.</div>
          </div>

          <div style={{display:'flex', gap:8, flexWrap:'wrap'}}>
            <Button size="l" view="action" type="submit" loading={busy} disabled={busy}>
              Обновить пароль
            </Button>
            <Button size="l" view="normal" type="button" disabled={busy} onClick={onCancel}>
              Отмена
            </Button>
          </div>
        </form>
