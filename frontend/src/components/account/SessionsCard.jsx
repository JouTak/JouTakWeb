...
import {Button, Label, Loader, Switch, Tooltip, DropdownMenu, Modal, Text} from '@gravity-ui/uikit';
import {
...
function reasonBadge(reason) {
  ...
  return <Label size="s" theme={r.theme}>{r.text}</Label>;
}
...
function normalizeSession(raw) {
  ...
  const created = raw.created ?? raw.created_at ?? raw.login_time ?? raw.start ?? null;
  const lastSeen = raw.last_seen ?? raw.last_activity ?? raw.updated_at ?? created ?? null;
  const expires = raw.expires ?? raw.expiry ?? raw.expire_at ?? null;
  const current = !!(raw.current ?? raw.is_current ?? raw.this_device ?? false);
  const ended = raw.revoked ?? raw.ended ?? raw.logged_out ?? raw.is_terminated ?? raw.is_expired ?? false;
  const reason =
    raw.revoked_reason ??
    raw.reason ??
...
export default function SessionsCard() {
  ...
  async function load() {
    ...
    try {
      ...
      const norm = (Array.isArray(raw) ? raw : (raw?.results || raw?.sessions || [])).map(normalizeSession);
  ...
  setSessions(arr => arr.map(s => (
    ids.has(s.id) ? {...s, revoked: true, revoked_reason: 'bulk_except_current'} : s
  )));
  return res;
}
  ...
  try {
    if (confirmAction === 'revoke-one' && targetSessionId) {
      ...
      setSessions(arr => arr.map(s => s.id === r.id ? {...s, revoked: true, revoked_reason: r.revoked_reason || 'manual'} : s));
  ...
  return (
    <section style={cardStyle}>
      ...
      <div style={{opacity:0.8}}>
        Отображаем {showHistory ? 'сессии за последние 48 часов (включая завершённые)' : 'активные сессии за последние 48 часов'}.
      </div>
      ...
      {loading ? (
        ...
        <div style={{display:'grid', gap:8}}>
          {sessions.map((s) => {
            ...
            if (s.current && !s.revoked) {
              actions.push({text: 'Выйти здесь', theme: 'danger', action: askSignoutCurrent});
            }
            if (!s.current && !s.revoked) {
              actions.push({text: 'Завершить', theme: 'danger', action: () => askRevokeOne(s.id)});
            }
            return (
              <div key={s.id} style={{
                ...rowBetween,
                ...
                background: s.current
                  ? 'rgba(13,148,16,0.08)'
                  : (s.revoked ? 'rgba(239,68,68,0.08)' : 'transparent')
              }}>
                <div style={{display:'grid', gap:4}}>
                  <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
                    <b>{s.current ? 'Это устройство' : 'Устройство'}</b>
                    {s.current && <Label size="s" theme="success">Текущая</Label>}
                    {s.revoked && <Label size="s" theme="danger">Завершена</Label>}
                  ...
                  <div style={{opacity:0.7, fontSize:12}}>
                    Истекает: {s.expires ? new Date(s.expires).toLocaleString() : '—'}
                  </div>
                </div>

                <div style={{display:'flex', gap:8}}>
                  {!s.current && !s.revoked && (
                    <Button view="outlined-danger" onClick={() => askRevokeOne(s.id)}>
                      Завершить
                    </Button>
                  )}
                  <DropdownMenu
                    size="m"
                    renderSwitcher={(props) => <Button {...props} view="outlined">Действия</Button>}
                    items={actions.length ? actions : [{text: 'Нет доступных действий', disabled: true}]}
        ...
        <div style={{opacity:0.8}}>
          {showHistory ? 'Сессий за 48 часов не найдено.' : 'Активных сессий не найдено.'}
        </div>
      )}
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} size="s">
        <div style={{padding:16, display:'grid', gap:12}}>
          ...
          <Text>
            ...
            {confirmAction === 'signout-current' && 'Выйти из аккаунта на этом устройстве?'}
            {confirmAction === 'revoke-all-except-current' && 'Завершить все сессии, кроме текущей?'}
          </Text>
          <div style={{display:'flex', gap:8, justifyContent:'flex-end'}}>
            <Button view="flat" onClick={() => setConfirmOpen(false)}>Отмена</Button>
            <Button view="action" onClick={runConfirmed}>Подтвердить</Button>
          </div>
        </div>
      </Modal>
    </section>
