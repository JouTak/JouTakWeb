...
export function setupAxiosInterceptors(onHardLogout = () => {}) {
  ...
  axios.interceptors.response.use(
    ...
    async (error) => {
      ...
      const isSessionExchange = config?.url?.includes('/api/auth/jwt/from_session');
...
async function postWithSession(url, body) {
  ...
  try {
    const resp = await axios.post(url, body, {...withSessionHeaders(st), __noGlobal401: true});
    const st2 = takeSessionToken(resp);
    if (st2 && st2 !== st) tokenStore.set({...tokenStore.get(), session_token: st2});
    return resp;
  } catch (e) {
    const r = e?.response;
    ...
    if (r?.status === 401 && st3) {
      ...
      return await axios.post(url, body, {...withSessionHeaders(st3), __noGlobal401: true});
    }
    throw e;
  }
}
...
async function getWithSession(url, params) {
  const st = tokenStore.getSessionToken();
  try {
    const resp = await axios.get(url, {...withSessionHeaders(st), params, __noGlobal401: true});
    const st2 = takeSessionToken(resp);
    if (st2 && st2 !== st) tokenStore.set({...tokenStore.get(), session_token: st2});
    return resp;
  } catch (e) {
    ...
    if (r?.status === 401 && st3) {
      tokenStore.set({...tokenStore.get(), session_token: st3});
      return await axios.get(url, {...withSessionHeaders(st3), params, __noGlobal401: true});
    }
    throw e;
  }
}
...
async function delWithSession(url, params) {
  const st = tokenStore.getSessionToken();
  try {
    const resp = await axios.delete(url, { ...withSessionHeaders(st), params, __noGlobal401: true });
    const st2 = takeSessionToken(resp);
    if (st2 && st2 !== st) tokenStore.set({ ...tokenStore.get(), session_token: st2 });
    return resp;
  } catch (e) {
    ...
    if (r?.status === 401 && st3) {
      tokenStore.set({ ...tokenStore.get(), session_token: st3 });
      return await axios.delete(url, { ...withSessionHeaders(st3), params, __noGlobal401: true });
    }
    throw e;
  }
}
...
export async function doLogin({username, password}) {
  ...
  try { await jwtFromSession(); } catch {}
...
export async function getOAuthLink(provider, next = "/account/security#linked") {
  const {data} = await getWithSession(`${API_BASE}/oauth/link/${provider}`, {next});
  return {url: sanitizeUrl(data.authorize_url), method: data.method || 'POST'};
}
...
export async function revokeSessionHeadless(session_id, reason = "manual") {
  const resp = await delWithSession(`${API_BASE}/account/sessions/${encodeURIComponent(session_id)}`, { reason });
  return resp.data;
}
...
export async function changeEmail(new_email) {
  const {data} = await postWithSession(`${API_BASE}/account/email/change`, {new_email});
  return data;
}

export async function resendEmailVerification() {
  const {data} = await postWithSession(`${API_BASE}/account/email/resend`, null);
  return data;
}
...
async function patchWithSession(url, body) {
  const st = tokenStore.getSessionToken();
  try {
    const resp = await axios.patch(url, body, { ...withSessionHeaders(st), __noGlobal401: true });
    const st2 = takeSessionToken(resp);
    if (st2 && st2 !== st) tokenStore.set({ ...tokenStore.get(), session_token: st2 });
    return resp;
  } catch (e) {
    ...
    if (r?.status === 401 && st3) {
      tokenStore.set({ ...tokenStore.get(), session_token: st3 });
      return await axios.patch(url, body, { ...withSessionHeaders(st3), __noGlobal401: true });
    }
    throw e;
  }
}
